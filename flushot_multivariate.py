#!/usr/bin/env python3
"""
Flu Shot Multivariate Messaging Experiment

This script runs a multivariate experiment to compare the effectiveness of 7 different
message variants for encouraging flu shot uptake:

1. Control (Direct): Simple direct message
2. Social Proof: Emphasizing what others are doing
3. Authority: Citing expert recommendations
4. Scarcity/Urgency: Emphasizing limited availability
5. Personal Benefit: Highlighting individual benefits
6. Fear Appeal: Emphasizing risks of not getting vaccinated
7. Reciprocity/Community Benefit: Emphasizing protection of others

Each persona will respond to all 7 messages (within-subjects design).
"""
...
def run_multivariate_experiment(num_personas=50, use_standard_personas=True):
    """
    ...
    # Export to CSV
    csv_filename = f"flushot_multivariate_results_{session_id[:8]}.csv"
...
def analyze_results(df):
    ...
    try:
        ...
        if len(complete_data) >= 10:  # Need a reasonable sample size
            try:
...
def create_visualization(response_rates, filename="flushot_multivariate_results.png"):
    """
...
def main():
    ...
    df = run_multivariate_experiment(num_personas=50, use_standard_personas=True)
