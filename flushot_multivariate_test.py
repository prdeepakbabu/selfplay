#!/usr/bin/env python3
"""
Flu Shot Multivariate Messaging Experiment (Test Version)

This is a smaller version of the multivariate experiment with fewer personas and messages
...
# Define message variants (reduced set for testing)
MESSAGE_VARIANTS = [
    {
        "id": 1,
        "name": "Control (Direct)",
...
def run_multivariate_experiment(num_personas=5, use_standard_personas=True):
    """
    ...
    # Export to CSV
    csv_filename = f"flushot_multivariate_test_results_{session_id[:8]}.csv"
...
def create_visualization(response_rates, filename="flushot_multivariate_test_results.png"):
    """
    ...
    try:
        ...
        plt.title('Effect of Different Message Framings on Flu Shot Acceptance (Test)')
...
def main():
    ...
    df = run_multivariate_experiment(num_personas=5, use_standard_personas=True)
